    ...
    for item in temp_storage:
        ...
        if item != "n" and item[0:2] != "xa" and item[0:2] != "nt":
            ...
            if item == key_word_end:
                ...
                if (item.isdigit() == True or item[0].isdigit()) and ":" not in item:
...
def organize_visible_text_from_html(list_of_visible_data, list_of_metrics, key_feature_first_metric):
...
def turn_list_into_csv_file(list_to_write_to_csv, headers, resulting_file_location, resulting_csv_file_name):
...
def turn_html_data_into_csv_file(html_data, key_word_start, key_word_end, metrics, key_feature_first_metric, resulting_file_location, resulting_csv_file_name):
    if len(html_data) > 1:
        end_list = []
        for group in html_data:
            visible_text_from_html = get_visible_text_after_keyword_from_html(group, key_word_start, key_word_end)
            list_to_write_to_csv = organize_visible_text_from_html(visible_text_from_html, metrics, key_feature_first_metric)
            end_list.append(list_to_write_to_csv)
        return turn_list_into_csv_file(end_list, metrics, resulting_file_location, resulting_csv_file_name)
    else:
